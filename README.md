...
| AWS `aws_s3_bucket_object` | AWS provider `>= 4.0.0` | `aws_s3_object` | **Auto-fix:** rewrite labels + refs; `moved` emitted only when module's `required_version` admits Terraform 1.8+ (cross-type `moved` needs CLI 1.8 + provider `MoveResourceState`) |
| Kubernetes `_v1` rename family (20 resources) | Kubernetes provider `>= 2.0.0` | append `_v1` suffix | **Auto-fix:** rewrite labels + refs ONLY. `moved` is *not* auto-emitted — schemas diverge between unversioned and `_v1` variants, `MoveResourceState` support varies per provider version, user verifies migration via `terraform plan` and authors a `moved` block manually if safe |
