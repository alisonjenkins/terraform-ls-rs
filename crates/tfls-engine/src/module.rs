...
pub fn module_under_test_dir(test_uri: &Url) -> Option<PathBuf> {
...
pub fn module_scope_dir(uri: &Url) -> Option<PathBuf> {
...
/// and vice-versa.
pub fn open_peers_in_scope(state: &StateStore, changed_uri: &Url) -> Vec<Url> {
...
pub fn module_supports_terraform_data(state: &StateStore, primary_uri: &Url) -> bool {
...
pub fn module_supports_templatefile(state: &StateStore, primary_uri: &Url) -> bool {
...
pub fn module_supports_locals_replacement(state: &StateStore, primary_uri: &Url) -> bool {
...
pub fn module_sensitive_variables(
    state: &StateStore,
    primary_uri: &Url,
...
pub fn module_providers_with_default_tags(
    state: &StateStore,
    primary_uri: &Url,
) -> std::collections::HashSet<String> {
    let mut out = std::collections::HashSet::new();
...
pub struct ModuleOutputResolver<'a> {
    pub state: &'a StateStore,
    pub caller_dir: PathBuf,
    pub cache: &'a ModuleOutputCache,
}
...
impl tfls_diag::unknown_value::ModuleOutputLookup for ModuleOutputResolver<'_> {
    fn output_apply_time(&self, module_label: &str, output: &str, rest: &[&str]) -> Option<bool> {
        ...
        let schema_lookup = StateStoreSchemaLookup { state: self.state };
...
pub fn module_unknown_inputs_for_dir(
    state: &StateStore,
    target_dir: &Path,
) -> tfls_diag::ModuleUnknownInputs {
...
pub fn module_constraint_for_provider(
    state: &StateStore,
    primary_uri: &Url,
    provider_name: &str,
) -> Option<String> {
...
pub fn module_locked_provider_version(
    state: &StateStore,
    primary_uri: &Url,
    provider_name: &str,
...
pub fn location_in_dir(loc: &SymbolLocation, dir: &Path) -> bool {
...
pub fn lookup_child_module_symbol(
    state: &StateStore,
    child_dir: &Path,
    kind: SymbolKind,
    name: &str,
...
pub struct StateStoreSchemaLookup<'a> {
    pub state: &'a StateStore,
}
...
pub fn required_providers_local_to_name(
            ...
            for entry in rp_block.body.iter() {
                ...
                if let Expression::Object(obj) = &attr.value {
                    for (key, value) in obj.iter() {
                        if let Some(k) = required_providers_object_key_as_str(key) {
                            if k == "source" {
                                if let Some(s) =
                                    required_providers_expr_literal_string(value.expr())
                                {
                                    return required_providers_parse_source_provider_name(&s)
...
fn required_providers_object_key_as_str(key: &hcl_edit::expr::ObjectKey) -> Option<&str> {
...
fn required_providers_expr_literal_string(expr: &hcl_edit::expr::Expression) -> Option<String> {
    use hcl_edit::expr::Expression;
    match expr {
...
fn required_providers_parse_source_provider_name(src: &str) -> Option<String> {
